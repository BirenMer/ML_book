...
class RNN:
    ...
    def forward(self, inputs, hprev):
        ...
        for t in range(len(inputs)):
            xs[t] = np.zeros_init(self.vocab_size, 1)
            ...
            )  # unnormalised log probs for next char
            ycap[t] = self.softmax(os[t])  # applying softmax to get the probability for the next character.
        return xs, hs, ycap
    ...
    def update_model(self,dU,dV,dW,db,dc):
        for param,d_param in zip([self.U,self.W,self.V,self.b,self.c],[dU,dV,dW,db,dc]):
